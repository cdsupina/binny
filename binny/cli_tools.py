...
# Parse arguments to check if the program should be run in debug mode
parser = argparse.ArgumentParser()
_ = parser.add_argument("--debug", "-d", action="store_true", help="Enable debug mode")
...
def parse_and_print_message(
    message: Message, console: Console, debug_enabled: bool = False
):
    """
    Parse and print a message based on its type and content.
    """

    if isinstance(message, SystemMessage) and debug_enabled:
        if message.subtype == "compact_boundary":
            # Message printed on compact of conversation
            print_rich_message(
                "system",
            ...
            # Message printed on conversation init
            print_rich_message("system", json.dumps(message.data, indent=2), console)
    elif isinstance(message, AssistantMessage):
        for block in message.content:
            if isinstance(block, TextBlock):
                # Message printed of assistant response
                print_rich_message("assistant", block.text, console)
            elif isinstance(block, ToolUseBlock) and debug_enabled:
                # Message notifying of tool use
                print_rich_message(
                    "tool_use", f"Tool: <{block.name}> \n\n {block.input}", console
                )
            elif isinstance(block, ThinkingBlock):
                # Message notifying the agent is thinking
                print_rich_message("assistant", "Thinking...", console)
    elif isinstance(message, UserMessage):
        for block in message.content:
            # Message notifying a tool result
            # User in UserMessage refers to the caller of the tool
            if isinstance(block, ToolResultBlock) and debug_enabled:
                formatted_content = format_tool_result(block.content)
                print_rich_message("tool_result", formatted_content, console)
    elif isinstance(message, ResultMessage) and debug_enabled:
        # Prints the session stats showing the details about the session resulting from the query
